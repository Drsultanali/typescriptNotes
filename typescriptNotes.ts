...
variable1 = 750;
var variable2: number = 600;
variable2 = 360;
const variable3: number = 700;
// variable3 = 269;
// In block scope we can delare variable outside the blockScope with the keyword of var only;
{
    let variable4: number = 400;
    var variable5: number = 800;
    const variable6: number = 1000;
}
//console.log(variable1, variable2, variable3);
function positive() {// In function scope/ local scope we cannot declare any variable(var, let, const) outside the block;
    let variable7: number = 100;
    var variable8: number = 200;
    const variable9: number = 300;
}
...
let sumResult: number = sum(10, 30);
...
let resultSum: number = sumArrow(205, 36);
...
let studentsRollNo: number[] = [100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110]// and so on as many you want to store.
let studentNames: string[] = ['Ali', 'Musa', 'Karim', 'Afzal', 'Sultan', 'Ammar', 'Humna', 'Hira']// and so on as many you want to store.
...
let numVariables: number[] = [3600, 5400, 2518, 9825, 6452, 8735, 7234, 1679, 5269]
let sumOfVariables: number = numVariables[2] + numVariables[5];//sum of 2nd index 2518 and 5th index 8735
console.log("sumOfVariables:", sumOfVariables);

// You can also add/concatenate strings from array
let strVariables: string[] = ['What is your name:', 'Sultan Ali', 'S/o', 'Mubarak Ali']
let concatOfStr: string = strVariables[0] + ' ' + strVariables[1] + ' ' + strVariables[2] + ' ' + strVariables[3];
console.log("concatenation of strings", concatOfStr);

//Add delete values from array:
let shoppingList: string[] = ['T-shirt', 'Jeans', 'Belt', 'Shoes', 'Waist Coat', 'Tie', 'Socks']
...
let toDoList: string[] = ['Get up early in the morning', 'Say Fajar prayer', 'Get shower', 'Do breakfast', 'Take medicine','Petrol check', 'Get laptop bag', 'Get Audit 2023 file', 'call to Mr. Mujtaba Anwar', 'Have lunch with Mr Mujtaba']
